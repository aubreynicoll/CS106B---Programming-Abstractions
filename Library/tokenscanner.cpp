 ...
 * 
 * THIS FILE IS FROM STANFORD C++ LIBRARY
 * 
 * @version 2016/11/26
...
void TokenScanner::verifyToken(const std::string &expected)
{
  ...
  if (token != expected)
  {
    std::string msg = "TokenScanner::verifyToken: Found \"" + token + "\"" + " when expecting \"" + expected + "\"";
    if (!buffer.empty())
    {
      msg += "\ninput = \"" + buffer + "\"";
    }
...
std::string TokenScanner::scanNumber()
{
  ...
  while (state != FINAL_STATE)
  {
    ...
    case INITIAL_STATE:
      if (!isdigit(ch))
      {
        std::cout << "TokenScanner::scanNumber: internal error: illegal call" << std::endl;
      }
...
std::string TokenScanner::scanString()
{
  ...
  while (true)
  {
    ...
    if (ch == EOF)
    {
      std::cout << "TokenScanner::scanString: found unterminated string" << std::endl;
    }
    if (ch == delim && !escape)
    {
      break;
